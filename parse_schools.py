...
def extract_geslaagden_per_profiel(content):
    ...
    sections = re.split(r'<tr><th>(havo|vwo|vmbo(?:-\(g\)t)?)</th>', content, flags=re.IGNORECASE)
    ...
    for i in range(1, len(sections), 2):
        ...
        if rows:
            ...
            for profiel, deelnemers, geslaagd in rows:
                ...
                if profiel and profiel not in ['Profiel', 'Totaal', ''] and \
                   any(p in profiel for p in ['Cultuur', 'Economie', 'Natuur', 'Maatschappij', 'Techniek', 'Gezondheid']):
...
def extract_tevredenheid_vragen(content):
    ...
    if leerling_dialog:
        ...
        vwo_section = re.search(r'<h4>vwo</h4>(.*?)(?:</section>|</dialog>)', dialog_content, re.DOTALL)
        if vwo_section:
...
def main():
    ...
    # Process regular school result files
    result_html_files = [f for f in base_dir.glob('*.html') if not f.name.startswith('tevredenheid-')]
    ...
    for html_file in result_html_files:
        ...
        try:
            ...
            output_name = html_file.stem + '-results.json'
    ...
    tevredenheid_files = list(base_dir.glob('tevredenheid-*.html'))
