...
def load_school_data(con, school_id, data):
    ...
    con.execute("""
        ...
        VALUES (?, ?, ?, ?, ?)
    """, [school_id, school.get('name'), school.get('address'),
          school.get('postalCode'), school.get('city')])
    ...
    for item in examencijfers:
        con.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?)
        """, [school_id, item.get('schooljaar'), item.get('centraalExamen'),
              item.get('centraalExamenVergelijking'), item.get('schoolExamen'),
              item.get('eindcijfer')])
    ...
    for item in geslaagden:
        con.execute("""
            ...
            VALUES (?, ?, ?, ?)
        """, [school_id, item.get('profiel'), str(item.get('deelnemers', '')),
    ...
    if 'onderwijspositie' in oordeel:
        ...
        con.execute("""
            ...
            VALUES (?, ?, ?, ?, ?)
        """, [school_id, 'onderwijspositie', item.get('inspectienorm'),
              item.get('schoolwaarde'), item.get('periode')])
    ...
    if 'onderbouwsnelheid' in oordeel:
        ...
        con.execute("""
            INSERT INTO oordeel_inspectie (school_id, indicator, inspectienorm, schoolwaarde, periode)
            VALUES (?, ?, ?, ?, ?)
        """, [school_id, 'onderbouwsnelheid', item.get('inspectienorm'),
              item.get('schoolwaarde'), item.get('periode')])
    ...
    if bovenbouwsucces:
        con.execute("""
            INSERT INTO oordeel_inspectie (school_id, indicator, inspectienorm, schoolwaarde, periode)
            VALUES (?, ?, ?, ?, ?)
        """, [school_id, 'bovenbouwsucces', bovenbouwsucces.get('inspectienorm'),
              bovenbouwsucces.get('schoolwaarde'), bovenbouwsucces.get('periode')])
    ...
    for metric, items in trends.items():
        for item in items:
            con.execute("""
                ...
                VALUES (?, ?, ?, ?, ?)
            """, [school_id, metric, item.get('schooljaar'),
                  item.get('cijfer'), item.get('vergelijking')])
...
def main():
    base_dir = Path('/Users/evert/projects/scholenopdekaart')
    ...
    json_files = sorted([f for f in base_dir.glob('*-results.json')
                         if not f.name.startswith('tevredenheid-')])
    ...
    tevredenheid_files = sorted(base_dir.glob('tevredenheid-*-results.json'))
    ...
    # Show summary
    print("\n--- Database Summary ---")
    for table in ['schools', 'doorstroom_onderbouw', 'doorstroom_bovenbouw',
                  'schooladvies', 'slagingspercentage', 'examencijfers',
                  'geslaagden_per_profiel', 'oordeel_inspectie',
                  'tevredenheid_trend', 'tevredenheid_vragen']:
